...
export const createFermions = (proton, neutron, array) => {
    ...
    for (let i = 0; array.length < proton; i++) {
        ...
        if ((Math.pow(distanceX, 2) + Math.pow(distanceY, 2)) < Math.pow(size / 2, 2)) {
            array.push(new Proton(window.innerWidth / 2 + distanceX - 150, window.innerHeight / 2 + distanceY - 100));
        }
    }
    for (let i = 0; (array.length - proton) < neutron; i++) {
        ...
        const distanceY = (Math.random() - 0.5) * size;
        if ((Math.pow(distanceX, 2) + Math.pow(distanceY, 2)) < Math.pow(size / 2, 2)) {
            array.push(new Neutron(window.innerWidth / 2 + distanceX - 150, window.innerHeight / 2 + distanceY - 100));
        }
    }
    array.sort(() => Math.random() - 0.5);
};
//# sourceMappingURL=createFermions.js.map