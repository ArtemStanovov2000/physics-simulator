...
const OFFSET = 5;
export const createElectronOrbital = (radius, count, array) => {
    for (let i = 0; i < count; i++) {
        const poindOffset = Math.random() * 2 * Math.PI;
        const xOffset = Math.sin(poindOffset) * radius + window.innerWidth / 2 + Math.random() * OFFSET - OFFSET / 2 - 150;
...
//# sourceMappingURL=createElectronOrbital.js.map